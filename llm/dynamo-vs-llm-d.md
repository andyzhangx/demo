...
## KAITO 整合建议：llm-d 更适合

[KAITO](https://github.com/kaito-project/kaito) 作为 Kubernetes 原生的 AI 推理 operator，如果要实现 Prefill/Decode 分离功能，**llm-d 是更合适的整合对象**。

### 选择 llm-d 的核心理由

1. **Kubernetes 原生架构一致**：KAITO 是 K8s operator，llm-d 直接构建在 K8s Gateway API / InferencePool CRD 之上，共享同一个控制面（K8s API），无需引入额外的服务发现组件。Dynamo 自带 etcd/NATS 控制面，与 KAITO 的 K8s operator 模型存在架构冲突。

2. **松耦合的 P/D 分离模型**：llm-d 的 P/D 协调通过 Decode 端 sidecar 实现，与 KAITO 的 Pod 编排模型天然兼容——KAITO 管 Pod 生命周期和 GPU 分配，llm-d sidecar 管 P/D 调度和 KV 传输。Dynamo 的 P/D 是框架内置的，整合需要引入整个 Dynamo 运行时。

3. **硬件中立**：KAITO 支持多种 GPU SKU，llm-d 显式支持 NVIDIA GPU、Intel XPU、Google TPU。Dynamo 强绑 NVIDIA 硬件优化（NVLink/NVSwitch）。

4. **声明式可扩展**：llm-d 的 SchedulingProfile 通过 YAML 编排 filter/scorer 链，KAITO 可通过 CRD 参数映射到不同调度策略，无需修改代码。Dynamo 需要修改 Rust 逻辑。

5. **社区方向对齐**：llm-d 由 Red Hat + IBM 社区主导，与 KAITO 的开源 K8s 生态定位一致。Gateway API + Inference Extension (IGW) 已是 K8s SIG 的发展方向。

### 建议的整合路径

```
KAITO Workspace CRD（扩展 P/D 模式声明）
  ├── Prefill Pool（KAITO 编排 prefill Pod + GPU 分配）
  ├── Decode Pool（KAITO 编排 decode Pod + GPU 分配）
  ├── llm-d EPP（路由层，通过 InferencePool CRD 对接）
  └── llm-d Sidecar（注入 decode Pod，处理 NIXL KV 传输）
```

- KAITO Workspace CRD 扩展 P/D 模式声明（prefill pool / decode pool）
- KAITO operator 负责 Pod 编排 + GPU 分配
