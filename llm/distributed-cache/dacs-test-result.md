...
### `qwen3-coder-30b-a3b-instruct-ccx24-0`

| Item | Value |
|---|---|
| Date (UTC) | 2026-08-05 |
| Node | `aks-ws824879e3f-28114834-vmss000000` |
| Model source | `az://pvc-58e234e9-ebc3-4dda-b4c6-451243e48251/Qwen/Qwen3-Coder-30B-A3B-Instruct` |
| Load format | `runai_streamer` |
| Cache / download path | DACS enabled, but most reads came from origin / remote client: `PrefetchCache=2037`, `RemoteCache=1955`, `RemoteClient=34325` |
| RunAI streamer time | 68.96s |
| **Model download throughput** | **844.5 MiB/s** |
| Model loading time | 70.11s |
| Pod Ready time | 5m36s |
| Conclusion | Same streaming mechanism, but cache hit ratio was low, so most model chunks were fetched from the remote source. |

### `qwen3-coder-30b-a3b-instruct-zdz9x-0`

| Item | Value |
|---|---|
| Date (UTC) | 2026-08-06 |
| Node | `aks-ws4d1d2affe-11877758-vmss000000` |
| Model source | `az://pvc-58e234e9-ebc3-4dda-b4c6-451243e48251/Qwen/Qwen3-Coder-30B-A3B-Instruct` |
| Load format | `runai_streamer` |
| Cache / download path | DACS enabled, and all model chunks were served from distributed cache: `PrefetchCache=0`, `RemoteCache=38317`, `RemoteClient=0`, `ZeroCopy=786` |
| RunAI streamer time | 25.88s |
| **Model download throughput** | **2.2 GiB/s** |
| Model loading time | 27.33s |
| Pod Ready time | 6m40s |
| Conclusion | Same streaming mechanism, but this run was effectively a hot-cache load, so model transfer/load was much faster. |

### `qwen3-coder-30b-a3b-instruct-kd2rg-0`

| Item | Value |
|---|---|
| Date (UTC) | 2026-08-06 |
| Node | `aks-wsed7ffa655-15239804-vmss000000` |
| Model source | `az://pvc-58e234e9-ebc3-4dda-b4c6-451243e48251/Qwen/Qwen3-Coder-30B-A3B-Instruct` |
| Load format | `runai_streamer` |
| Cache / download path | No DACS/cache injection visible on the pod; model was streamed directly from the Azure source using Workload Identity |
| RunAI streamer time | 34.41s |
| **Model download throughput** | **1.7 GiB/s** |
| Model loading time | 35.56s |
| Pod Ready time | 6m55s |
| Conclusion | Same RunAI streamer mechanism, but this pod appears to be a direct Azure-source load rather than a distributed-cache hit. |
...
| Fastest path | `zdz9x-0` was fastest because it hit hot DACS distributed cache. |
| **Model download throughput** | **`ccx24-0`: 844.5 MiB/s; `kd2rg-0`: 1.7 GiB/s; `zdz9x-0`: 2.2 GiB/s** |
